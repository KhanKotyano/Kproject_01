...
unsigned int GetMaxFCostIndex(IntArray *node_str, ASNode *path_array){
  //node_str->array;
  RedoIntArray(node_str);
  unsigned int hight_index = 0;
  int prev_max_val = path_array[node_str->array[0]].F;
  ...
  //printf("MAX INDEX :%u \n", hight_index);
  //printf("MAX USED INDEX :%llu\n", (node_str->used)-1);
  int return_val = (int)(node_str->array[hight_index]);
  ...
  *ASNode_array = (ASNode *)malloc(sizeof(ASNode) * (from_grid->height * from_grid->width));
  ...
  for(u32 y = 0;y<total_height;y++){
    for(u32 x = 0;x<total_width;x++){
      ...
      (*ASNode_array)[cur_index].can_move = 1;
      (*ASNode_array)[cur_index].F = (int)(from_grid->grid[y][x].nature.terrain->move_cost);
...
void AStar2(IntArray *open,IntArray *closed, GridVector2D max_pos, ASNode *path_array, GridVector2D from_node, GridVector2D target_node
  ...
  while(open->used){
    ...
    unsigned int current_index = GetMaxFCostIndex(open, path_array);//index2D of path_array
    //RedoIntArray(open);
    ...
    for(int i = 0;i < path_array[current_index].border_nodes_size;i++){
      ...
      printf("cur_border_index:%d \n", cur_border_index);
      if(!(path_array[cur_border_index].can_move) || IsIndexInArray(closed, cur_border_index)){
        continue;
      } 
      if(!IsIndexInArray(open, cur_border_index)){
        //printf("true\n");
        path_array[cur_border_index].parent_index = current_index;
        path_array[cur_border_index].F = path_array[cur_border_index].F + path_array[current_index].F;
